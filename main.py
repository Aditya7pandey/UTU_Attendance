...
def generate_ai_insights():
    try:
        ...
        if not os.path.exists("students_data.csv") or os.path.getsize("students_data.csv") == 0:
        ...
        return f"Error: Required file not found ({e}). Please ensure 'Attendance.csv' and 'students_data.csv' exist."
    except requests.exceptions.RequestException as e:
        return f"Error communicating with Groq API: {e}. Please check your API key and network connection."
...
def plot_attendance_comparison(selected_students=None):
    try:
        ...
        attendance_df = pd.read_csv(ATTENDANCE_FILE)
        if attendance_df.empty:
            st.warning("No attendance records found for plotting.")
            return None
        ...
        st.error(f"Error: Required file not found ({e}). Please ensure 'Attendance.csv' and 'students_data.csv' exist.")
        return None
    except Exception as e:
...
def mark_attendance_and_reward(student_name, frame):
    ...
    existing_record = df[(df["Name"].str.upper() == student_name.upper()) & (df["Date"] == dateString)]
...
with col3:
    registered_faces = len([f for f in os.listdir(TRAINING_IMAGES_DIR) if f.lower().endswith(('.png', '.jpg', '.jpeg'))])
    st.markdown(f"""
    <div class="metric-card">
...
with col4:
    if os.path.exists(REWARDS_FILE):
        ...
        gold_badges = len(rewards_df[rewards_df["Badge"] == "Gold"]) if not rewards_df.empty else 0
    else:
        gold_badges = 0
    st.markdown(f"""
    <div class="metric-card">
...
if st.sidebar.button("🤖 AI Attendance Insights", key="ai_insights"):
    ...
    all_student_names = []
    if os.path.exists("students_data.csv") and os.path.getsize("students_data.csv") > 0:
        students_df = pd.read_csv("students_data.csv")
